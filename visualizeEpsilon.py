...
""" Visualizes the results of calcEpsilon. """
...
def main(seedname, direction, schemes, visType, normalize):
    ...
    for k in data.keys():
        ...
        if not schemes is None and not kLast in schemes:
            continue
        keys[k] = kLast
    visDict = { "Re" : dict(f=lambda eps, omega : np.real(eps), label=r"Re $\epsilon_{{{}}}$ [arb.u.]"),
                "Im" : dict(f=lambda eps, omega : np.imag(eps), label=r"Im $\epsilon_{{{}}}$ [arb.u.]"),
                "sigma" : dict(f=lambda eps, omega : omega * np.imag(eps) / 4 / np.pi, label=r"Re $\sigma_{{{}}}$ [arb.u.]"),
              }
    f = visDict[visType]['f']
...
def createParser():
    ...
    parser.add_argument('-s', '--schemes', type=lambda s: s.split(","),
                        ...
                        default=None)
    parser.add_argument('-t', '--type', dest='visType', default="Re", choices=["Re", "Im", "sigma"])
